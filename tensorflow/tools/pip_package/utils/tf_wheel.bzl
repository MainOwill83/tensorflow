...
load("@python_version_repo//:py_version.bzl", "HERMETIC_PYTHON_VERSION", "MACOSX_DEPLOYMENT_TARGET", "WHEEL_COLLAB", "WHEEL_NAME")
...
def _unpacked_tf_wheel_impl(ctx):
...
unpacked_tf_wheel = rule(
    implementation = _unpacked_tf_wheel_impl,
    attrs = {
        "wheel": attr.label(mandatory = True, allow_single_file = True),
        "zipper": attr.label(default = Label("@bazel_tools//tools/zip:zipper"), cfg = "exec", executable = True),
    },
)
